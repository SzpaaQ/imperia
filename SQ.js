...
var SQ_autoattack 		= function(){if(SQ_attackWorking == false){
	SQ_otherInterval = setInterval(function(){spy()},1000);$('.SQ_startattack').find('.ui-buttons').css('background-color','rgba(255,255,255,.7)');SQ_attackWorking = true;}else{clearInterval(SQ_otherInterval);$('.SQ_startattack').find('.ui-buttons').css('background-color','rgba(0,0,0,0)');SQ_attackWorking = false;}}
var spy = function(lvl){if($('#widget-missions').find('.mission-my').length) return;if(!lvl) lvl = 7;xajax_doMassSpy(container.open({saveName:'mass-spy-report-100'+lvl, title:'Masowe szpiegostwo: Niezależne miasto'}),{'uId':100+lvl,'isUserHolding':'1', 'page': 1});
	clearInterval(SQ_otherInterval);SQ_otherInterval=setInterval(function(){SQ_setVictim()},1000)}}
var openTwierdza 		= function(){
	if($('#widget-missions').find('.mission-my').length) return;$('body').trigger(jQuery.Event( 'keyup', { keyCode: 67, which: 67 } ));
	clearInterval(SQ_otherInterval);SQ_otherInterval=setInterval(function(){SQ_setVictim()},1000)}
